...
function injectNav(activePage) {
  ...
  const leftLinks = ['index', 'how-it-works', 'services']
    .map(id => pages.find(p => p.id === id))
    .map(p => `<li><a href="${p.href}"${activePage === p.id ? ' class="active"' : ''}>${p.label}</a></li>`)
    .join('');

  const rightLinks = ['packages', 'blog']
    .map(id => pages.find(p => p.id === id))
    .map(p => `<li><a href="${p.href}"${activePage === p.id ? ' class="active"' : ''}>${p.label}</a></li>`)
    .join('');
  ...
  document.body.insertAdjacentHTML('afterbegin', `
    ...
    <div class="mobile-menu" id="mobileMenu">
      ${pages.map(p => `<a href="${p.href}" class="mobile-nav-link${p.id === activePage ? ' active' : ''}">${p.label}</a>`).join('')}
    </div>
  `);
